...
module_ortho: {
    title: "Lameness & Joints",
    group: "Tracker",
    category: "Modules",
    audience: ["vet", "owner"],
    difficulty: 2,
    textOwner: "This module keeps a record of a limp or a joint problem: one record per problem, with a daily score for how lame your pet was. The score runs from 0 to 4 and describes what you can actually see — 0 is walking normally, 2 is an obvious limp they are still putting weight on, and 4 is not using the leg at all. That is deliberate: nobody can measure how much an animal hurts, but you can tell whether the foot is going down. Alongside the score you record which legs were affected that day, how hard it was to get up, what seemed to make it worse, and what pain relief was actually given — which is not always the same as what was prescribed, and the difference often explains a bad week. Which legs matters more than it sounds: a pet taking the weight off a sore back leg will start to favour a front one, and noticing that shift is genuinely useful. Score the good days as well as the bad ones. Lameness comes and goes, and it is the pattern over weeks that means something, not any single day. The scores are shown against how far your pet walked that day, because a mild limp after a two-hour walk and the same limp after lying in all day are not the same thing. What this module will not do is tell you what is causing the limp. There are a great many possible causes and telling them apart needs a vet to examine your pet, usually with x-rays — a score typed into a phone cannot do it, and a quiet fortnight here is not proof that a problem has resolved. If you were given a diagnosis you can record it, in your own words, so it travels with the rest of the history. What the record gives your vet is a fortnight or a year of consistent observations instead of a recollection at the consulting-room door, and that is what makes the examination more useful.",
    textClinical: "Per-problem orthopaedic record with a daily owner-scored lameness grade on the conventional 0–4 visual scale, worded observationally (weight-bearing status and limb use) rather than as a pain assessment. Each entry additionally captures limb(s) affected on the day — permitting detection of compensatory contralateral or forelimb loading — stiffness on rising, aggravating factors (post-rest versus post-exercise, cold, stairs, slippery flooring), and pain relief actually administered, recorded separately from the prescribed regime so compliance gaps are visible against symptom days. Standing fields hold chronicity (acute, intermittent, chronic, progressive), owner-reported diagnosis where one has been given, current analgesic plan and adjunctive management (exercise restriction, weight management, hydrotherapy, environmental modification). Lameness scores are presented against concurrent activity duration from the activity log; the score is never adjusted for activity — the two series are displayed in parallel for clinical interpretation. Unscored days are held as null and excluded from series and means rather than imputed as sound. Explicitly non-diagnostic and non-reassuring by design: all generated text is a statement about the record (e.g. flare-day counts within a rolling 14-day window) and never an aetiological or prognostic inference. Intended to supply objective serial data for analgesic titration, response monitoring and referral decisions where owner recall is otherwise unreliable."
},
