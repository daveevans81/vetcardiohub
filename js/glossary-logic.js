const glossaryEngine = {
    ...
    // Dynamic Filters
    selectedGroup: 'All', // e.g., Echo, Pharmacology, Supplements
    ...
    availableCategories() {
        ...
        if (this.selectedGroup !== 'All') {
            items = items.filter(item => (item.group || 'Echo') === this.selectedGroup);
        }
    ...
    glossaryArray() {
        ...
        return Object.entries(this.glossaryDatabase)
            ...
            // Removed the echo-only filter so ALL terms load!
            .sort((a, b) => {
    ...
    dynamicGlossaryFields() {
        ...
        Object.entries(this.activeGlossaryTerm).forEach(([key, value]) => {
            if (!hiddenFields.includes(key) && value) {
                const conf = fieldConfig[key] || { label: key.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase()), icon: "fa-circle-info", color: "#94a3b8" };
                ...
                fields.push({ id: key, label: conf.label, icon: conf.icon, color: conf.color, value: formattedValue });
            }
        });

        const sortOrder = ['description', 'textOwner', 'textClinical', 'view', 'method', 'reference'];
        fields.sort((a, b) => {
            let idxA = sortOrder.indexOf(a.id); let idxB = sortOrder.indexOf(b.id);
    ...
    filteredGlossary() {
        ...
        if (this.selectedGroup !== 'All') {
            list = list.filter(item => (item.group || 'Echo') === this.selectedGroup);
        }
        ...
        if (this.searchQuery.trim() !== '') {
            ...
            list = list.filter(item => 
                ...
                (item.textClinical && item.textClinical.toLowerCase().includes(q))
            );
        }
        return list;
    },
    ...
    startQuiz() {
        ...
        if (this.quizMode === 'mcq' && this.quizDeck.length > 0) this.generateMCQ();
    },
    ...
    markCard(status) {
        if (status === 'correct') this.sessionScore.correct++;
        else this.sessionScore.review++;
        this.nextQuizCard();
    },
    ...
    generateMCQ() {
        ...
        // Smart text extractor handles both Echo and Pharmacology terms
        const getDesc = (item) => item.description || item.textClinical || item.textOwner || '';
        const correctText = getDesc(correctCard);

        let uniquePool = this.glossaryArray().filter(item => 
            item.key !== correctCard.key && 
            getDesc(item).trim() !== '' && 
            getDesc(item).trim() !== correctText.trim()
        );
        
        const seenDescriptions = new Set();
        uniquePool = uniquePool.filter(item => {
            const text = getDesc(item);
        ...
        selectedDistractors.forEach(dist => {
            options.push({ text: getDesc(dist), isCorrect: false });
        });
    ...
    selectMCQAnswer(index) {
        ...
        if (this.mcqOptions[index].isCorrect) this.sessionScore.correct++;
        else this.sessionScore.review++;
    },

    nextQuestion() {
        this.currentQuizIndex++;
        if (this.currentQuizIndex >= this.quizDeck.length) this.startQuiz(); 
        else {
    ...
    initLearningHub() {
        ...
        this.learningMode = urlParams.get('mode') === 'glossary' ? 'glossary' : 'quiz';
        this.quizMode = urlParams.get('type') === 'flashcard' ? 'flashcard' : 'mcq';
        if (this.learningMode === 'quiz') this.startQuiz();
    },
    ...
    closeGlossary() {
        ...
        setTimeout(() => { this.activeGlossaryTerm = null; }, 300); 
    }
};