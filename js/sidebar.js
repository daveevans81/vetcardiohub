...
(function() {
  document.addEventListener("DOMContentLoaded", function() {
    ...
    if (isSeries) {
      // If in a series, get all modules in order
      displayPosts = posts
      ...
      // Otherwise, show featured articles
      displayPosts = posts.filter(p => p.featured).slice(0, 5);
    }

    // Related posts fallback (only if not in a series search)
    let relatedPosts = [];
